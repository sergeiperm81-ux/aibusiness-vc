/**
 * The five AI models the scan asks, with their logos, so a visitor sees at a
 * glance which assistants are checked. Logo shapes: Simple Icons (CC0) for
 * OpenAI, Anthropic, Gemini and Perplexity; LobeHub Icons (MIT) for Grok. The
 * marks belong to their owners and are used only to name the models asked.
 */
...
const LOGOS: readonly Logo[] = [
  ...
  },
  {
    name: "Perplexity",
    viewBox: "0 0 24 24",
    paths: ["M22.3977 7.0896h-2.3106V.0676l-7.5094 6.3542V.1577h-1.1554v6.1966L4.4904 0v7.0896H1.6023v10.3976h2.8882V24l6.932-6.3591v6.2005h1.1554v-6.0469l6.9318 6.1807v-6.4879h2.8882V7.0896zm-3.4657-4.531v4.531h-5.355l5.355-4.531zm-13.2862.0676 4.8691 4.4634H5.6458V2.6262zM2.7576 16.332V8.245h7.8476l-6.1149 6.1147v1.9723H2.7576zm2.8882 5.0404v-3.8852h.0001v-2.6488l5.7763-5.7764v7.0111l-5.7764 5.2993zm12.7086.0248-5.7766-5.1509V9.0618l5.7766 5.7766v6.5588zm2.8882-5.0652h-1.733v-1.9723L13.3948 8.245h7.8478v8.087z"],
  },
  {
    name: "Grok",
    viewBox: "0 0 24 24",
...
/** Each model in its own brand colour. Gemini's mark is a gradient, drawn with an SVG gradient. */
const COLOURS: Readonly<Record<string, string>> = {
  OpenAI: "#000000",
  Anthropic: "#D97757",
  Gemini: "url(#gemini-gradient)",
  Perplexity: "#1FB8CD",
  Grok: "#000000",
};

/**
 * The five models as a full-width row of white tiles, each logo in its brand
 * colour: the models are the product, so they are shown large.
 */
export function ModelLogos() {
  return (
    <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
      {LOGOS.map((logo) => (
        <li key={logo.name} className="flex items-center justify-center gap-3 rounded-2xl bg-white px-4 py-6">
          <svg viewBox={logo.viewBox} aria-hidden className="h-10 w-10 shrink-0" fillRule="evenodd">
            {logo.name === "Gemini" && (
              <defs>
            ...
            {logo.paths.map((d) => (
              <path key={d.slice(0, 24)} d={d} fill={COLOURS[logo.name]} />
            ))}
          </svg>
          <span className="text-xl font-bold text-black">{logo.name}</span>
        </li>
      ))}
    </ul>
  );
