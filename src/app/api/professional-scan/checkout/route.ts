...
export async function GET(request: Request) {
  ...
  const buyLink = process.env.LEMONSQUEEZY_PRO_SCAN_CHECKOUT_URL?.trim();
  const apiKey = process.env.LEMONSQUEEZY_API_KEY?.trim();
  const storeId = process.env.LEMONSQUEEZY_STORE_ID?.trim();
  const variantId = process.env.LEMONSQUEEZY_PRO_SCAN_VARIANT_ID?.trim();
  if (!buyLink || !apiKey || !storeId || !variantId) return back("not_configured");
  const url = await createScanCheckout({ apiKey, storeId, variantId, buyLink }, preview.id, "person");
