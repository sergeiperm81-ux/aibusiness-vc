...
"mixups": [{"who": another person with the same or a similar name that an assistant brought up, one entry per person, with what tells them apart (field, employer, country), "saidBy": [labels], "links": up to 3 page addresses about that other person, copied exactly from that assistant's answer or its cited sources, or an empty list}]. One entry per person, never a group: an astrophysicist, an urban planner and an estate agent are three entries, each with its own links. "who" starts with what that person is, for example "An astrophysicist at the University of Oxford".

"disagreements": [{"topic": what they disagree on, "versions": [{"saidBy": one label, "says": that assistant's version}]}]. Only real conflicts about the same fact: two different current employers, a past role presented as current by one assistant, two different cities. At most 4.
...
"recommendations": 5 to 7 entries {"title": an action in a few words, "why": the gap in these answers that it closes, in one sentence, "steps": 2 to 4 concrete steps, each a full sentence that says exactly what to write or publish and where}. A step is never "improve" or "clarify" or "increase": it names the place and the text. For a namesake, steps look like: "Use one headline on every network: role, field, city, for example 'Investment analyst, Dubai real estate'."; "Add a middle initial or a field tag to the display name if a namesake works in the same field."; "Link the profiles to each other, so a model that finds one finds the rest."; "Publish one dated page that states name, role, employer and city in its first sentence." For missing reviews: ask two past clients for a LinkedIn recommendation this week. Never recommend resolving a disagreement unless it is listed under "disagreements". Base every one on a gap visible in the answers: a missing contact, no dated recent activity, assistants that could not find the person, a namesake they are confused with, a role that is out of date, no independent mentions. Write them as advice to the person the report is about, in the imperative ("Add...", "Publish..."). When a namesake came up, one recommendation must be about standing apart from that namesake: the same city, employer and field next to the name everywhere. Do not promise that any assistant will change its answer.`;
