---
title: "Walmart's AI Negotiates Better Deals Than Humans — Saving Billions"
...
1. **Analyzes the market.** Checks commodity prices, competitor pricing, and historical purchase data
2. **Sets targets.** Determines the ideal price range based on volume, quality requirements, and market conditions
3. **Makes the opening offer.** Typically 5-10% below the current contract price, supported by market data
4. **Handles counteroffers.** Evaluates supplier responses against predefined parameters. Adjusts based on logic, not emotion
5. **Closes or escalates.** If agreement is reached, finalizes the contract. If the supplier's position is outside acceptable parameters, escalates to a human buyer
