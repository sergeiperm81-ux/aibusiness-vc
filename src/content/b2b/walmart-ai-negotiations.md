...
1. **Analyzes the market.** Checks commodity prices, competitor pricing, historical purchase data
2. **Sets targets.** Determines the ideal price range based on volume, quality needs, and market conditions
3. **Makes the opening offer.** Typically 5-10% below current contract price, backed by market data
4. **Handles counteroffers.** Evaluates responses against predefined parameters. Adjusts based on logic, not emotion
5. **Closes or escalates.** Agreement reached? Finalize the contract. Supplier's position outside acceptable range? Hand off to a human buyer
